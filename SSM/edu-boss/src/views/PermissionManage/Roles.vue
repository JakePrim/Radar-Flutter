...
export default {
  ...
  methods: {
    ...
    loadRoles() {
      ...
      return axios.post('/role/findAllRoles',this.listQuery).then(res=>{
    ...
    //添加&修改角色
    handleSave() {
      axios.post('/role/saveOrUpdateRole',this.role).then(res=>{
        this.dialogVisible = false
        //刷新列表数据
        this.loadRoles()
      }).catch(()=>{
        this.$message('添加或修改失败')
      })
    },

    //删除角色
    handleDelete(row) {
      axios.get('/role/deleteRole?roleId='+row.id).then(res=>{
//刷新列表数据
        this.loadRoles()
      }).catch(()=>{
        this.$message('添加或修改失败')
