<template>
  <div class="app-container">
    ...
    <div class="pagination-container">
      <el-pagination background @size-change="handlePageSizeChange" @current-change="handleCurrentPageChange" layout="total, sizes,prev, pager, next,jumper" :current-page="listQuery.currentPage" :page-sizes="[5,10, 20]" :page-size="listQuery.pageSize" :total="total"></el-pagination>
    </div>
...
export default {
  ...
  methods: {
    ...
    getResourceList() {
      ...
      axios.post('/resource/findAllResource',listQuery).then(res=>{
    ...
    //方法2: 获取资源分类数据
    getResourceCateList() {
      axios.get('/resourceCategory/findAllResourceCategory').then(res=>{
        this.cateList = res.data.content
        for (let index = 0; index < this.cateList.length; index++) {
          const element = this.cateList[index];
    ...
    handleUpdate(row) {
      this.dialogVisible = true
      this.isEdit = true
      Object.assign(this.resource,row)
    },

    //删除资源
    handleDelete(row) {
      axios.get('/resource/deleteResource?id='+row.id).then(res=>{
        this.getResourceList()
      })
    },
