...
export default {
  ...
  methods: {
    ...
    //添加&修改资源分类
    handleSave() {
      axios.post('/resourceCategory/saveOrUpdateResourceCategory',this.resourceCategory).then(res=>{
        this.dialogVisible = false
        this.getCateList();
      }).catch(()=>{
        this.dialogVisible = false
        this.$message('添加或修改资源分类失败')
      })
    },

    //删除
    handleDelete(index, row) {
      axios.get('/resourceCategory/deleteResourceCategory?id='+row.id).then(res=>{
        this.getCateList()
      })
    }
  }
