  ...
  <section class="users">
    ...
    <el-form class="actions" :model="filter" label-position="top" inline>
      ...
      <el-form-item label="注册时间">
        <el-date-picker v-model="filter.resTime" type="daterange" start-placeholder="开始日期" end-placeholder="结束日期" range-separator="至" :picker-options="pickerOptions"></el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <!-- 分页 -->
    <el-pagination v-if="users.length" layout="total, sizes, prev, pager, next, jumper" @size-change="handlePageSizeChange" @current-change="handleCurrentPageChange" :current-page="page" :page-sizes="[10, 20, 30]" :page-size="size" :total="total" />
...
export default {
  ...
  methods: {
    ...
    loadUsers() {
      ...
      //数据查询
      axios.post('/user/findAllUserByPage',params).then(res=>{
    ...
    handleSelectRole(index, row) {
      ...
      //查询用户角色
      axios.get('/user/findUserRoleById?userId='+this.allocAdminId).then(res=>{
          const list = res.data.content
          this.allocRoleIds = []
          for (let index = 0; index < list.length; index++) {
            const element = list[index];
            this.allocRoleIds.push(element.id)
          }
      })
    },
    handleAllocDialogConfirm(){
      //allocRoleIds
      axios.post('/user/userContextRole',{
